...
This repository contains an exploratory analysis inspired by the study **"Fecal Metabolomic Signatures in Colorectal Adenoma Patients Are Associated with Gut Microbiota and Early Events of Colorectal Cancer Pathogenesis"** by Kim et al. (2020).

The project was developed as a learning-focused replication to explore different analytical techniques using data from the study. It was originally created in the context of a hackathon to practice and showcase analysis workflows in a time-constrained setting. It is intended for educational and skill-building purposes only.

## Project Purpose

The aim of this work is to:

- Revisit a published colorectal cancer-related multi-omics study through an exploratory lens.
- Practice applying different computational and statistical methods to real biological data.
- Build familiarity with workflows for integrating metabolite data and microbial genus count data.
- Compare and test different analysis approaches in a non-clinical, non-diagnostic setting.

## Data Types

This project uses the following data types from the study:
...
The main R packages used in this analysis are:
...
This repository is strictly exploratory. It does not aim to reproduce the original paper in a definitive way, establish new biological conclusions, or provide clinical interpretation.

The analysis was carried out to learn from an existing study design, experiment with methods, and better understand how different tools behave on this type of data, particularly within a hackathon-style project.

## Acknowledgment

This work is inspired by the following paper:

Kim et al. (2020), *Fecal Metabolomic Signatures in Colorectal Adenoma Patients Are Associated with Gut Microbiota and Early Events of Colorectal Cancer Pathogenesis*.

All credit for the original study, data generation, and scientific conclusions belongs to the original authors.

## Disclaimer

This repository is for learning purposes only. Any data used here comes from the referenced study and is used only as part of an educational exploratory analysis.
...
# Microbiome analysis and tables
library(dplyr)
library(tidyr)
library(knitr)
library(kableExtra)
...
In this section, I load the metabolomics, metagenomics, and metadata tables and bring them into a consistent format (sample IDs as rownames, features/taxa as columns).  
This step ensures that all downstream analyses operate on aligned samples across all data types.
...
Here I align sample IDs between omics data and metadata, and perform basic checks for missing samples or mismatches.  
This alignment is crucial so that group labels and covariates correspond correctly to each row of omics data.
...
```{r}
# Re-define helper for clarity later
align_data <- function(omics_data, metadata) {
  aligned_data <- omics_data[rownames(omics_data) %in% metadata$Sample, , drop = FALSE]
  idx <- match(rownames(aligned_data), metadata$Sample)
  metadata_aligned <- metadata[idx, ]
...
In this section, I explore the metabolomics data: first with basic quality control and PCA, then with differential analysis using limma.  
The aim is to see how metabolite profiles differ across control, adenoma, and carcinoma groups.

## 3.1 Quality control

Here I check overall intensity distributions and variability across samples to detect potential outliers or technical issues.  
These diagnostics help decide whether further normalization or filtering is needed.
...
PCA summarizes the main sources of variation in the metabolomics data and lets me visualize how samples group in a low-dimensional space.  
I use it here to see whether control, adenoma, and carcinoma samples separate based on their metabolite profiles.
...
Before interpreting molecular differences, I summarise basic demographic variables across groups.  
This helps check whether age, sex, race, or smoking status are imbalanced between control, adenoma, and carcinoma.
...
## 3.4 Differential analysis of metabolites (limma)

Here I use limma to identify metabolites that differ between groups (adenoma vs control, carcinoma vs control, carcinoma vs adenoma).  
This is a standard linear modelling framework that provides fold changes, adjusted p-values, and volcano plots.
...
## 3.5 Global test of metabolome–group association

Globaltest evaluates whether the full set of metabolites, as a whole, is associated with the group variable.  
Instead of focusing on single features, it asks if the entire metabolomic profile carries signal related to disease status.
...
# Subset to Control / Adenoma / Carcinoma
...
In this part I work with the genus-level count data, perform basic filtering and normalization, and examine alpha and beta diversity.  
The goal is to see how microbial composition varies with disease status.
...
# Remove samples with low total counts
...
Alpha diversity summarizes within-sample diversity (richness and evenness) of the microbiome.  
Here I compute Shannon index and richness per sample and compare distributions across groups.
...
## 4.2 Beta diversity and community structure

Beta diversity captures differences in community composition between samples.  
I use Bray–Curtis distances, PCoA plots, and PERMANOVA to assess how much of the between-sample variation is explained by group and covariates.
...
## 4.3 Differential abundance (ANCOM-BC)

Here I use ANCOM-BC to test for differential abundance of microbial taxa across groups while accounting for compositionality and covariates.  
The results are summarized with volcano plots and barplots of significant taxa.
...
out_meg <- ancombc2(
  data        = abundance_data,
  meta_data   = meta_data,
  fix_formula = "Study.Group + Age + Gender",
  group       = "Study.Group",
  p_adj_method = "BH",
  prv_cut      = 0.10,
  lib_cut      = 1000,
...
```{r}
# Barplots of significant taxa with effect sizes (Adenoma and Carcinoma)
df_fig_adenoma <- res %>%
  transmute(
    taxon,
...
In this section I jointly model metabolomics and metagenomics using DIABLO (mixOmics) to identify multi-omics signatures that discriminate between groups.  
The focus is on shared components across data blocks and on the variables that contribute most to these components.
...
pred_class <- pred$class$max.dist   # list with $metabolomics, $metagenomics
...
## Component 1 (using metabolomics predictions)
...
## 9. Joint table for both components
...
```{r}
# 10. Main contributing variables
...
# 3b. Cross-validation with misclassification error
set.seed(123)
...
# 5. Selected variables at lambda.min
...
```{r}
# Variables selected at lambda.min (repeated summary)
coef_1se <- coef(cv_full_auc, s = "lambda.min")

sel_1se <- as.matrix(coef_1se)
sel_1se <- sel_1se[-1, , drop = FALSE]  # remove intercept
